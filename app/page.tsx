...
const QuickRegisterModal = lazy(() =>
  import("@/components/quick-register-modal").then((module) => ({ default: module.QuickRegisterModal })),
)
const ClearDataModal = lazy(() =>
  import("@/components/clear-data-modal").then((module) => ({ default: module.ClearDataModal })),
)
const BarcodeScanner = lazy(() =>
  import("@/components/barcode-scanner").then((module) => ({ default: module.BarcodeScanner })),
)
const PremiumUpgradeModal = lazy(() =>
  import("@/components/premium-upgrade-modal").then((module) => ({ default: module.PremiumUpgradeModal })),
)
...
// Memoized components
const ProductCountItem = ({ item, onRemove }: { item: ProductCount; onRemove: (id: string) => void }) => (
...
const ProductTableRow = ({ product, barCode }: { product: Product; barCode?: BarCode }) => (
  <TableRow>
...
export default function InventorySystem() {
  ...
  const [currentProduct, setCurrentProduct] = useState<Product | TempProduct | null>(null)
  ...
  const productCountsStats = useMemo(() => {
    const totalLoja = productCounts.reduce((sum, item) => sum + item.quant_loja, 0)
    const totalEstoque = productCounts.reduce((sum, item) => sum + item.quant_estoque, 0)
    const totalSistema = productCounts.reduce((sum, item) => sum + item.saldo_estoque, 0)
  ...
  const handleScan = useCallback(() => {
    ...
    const tempProduct = tempProducts.find((tp) => tp.codigo_de_barras === scanInput)
    if (tempProduct) {
  ...
  const handleBarcodeScanned = useCallback(
    (barcode: string) => {
      ...
      setTimeout(() => {
        ...
        // Then check in temporary products
        const tempProduct = tempProducts.find((tp) => tp.codigo_de_barras === barcode)
        if (tempProduct) {
          setCurrentProduct(tempProduct)
          toast({
            title: "Produto temporário encontrado!",
  ...
  const handleQuickRegister = useCallback(
    (data: { codigo_de_barras: string; descricao: string; quantidade: string }) => {
  ...
  const calculateTotal = useCallback((quantLoja: number, quantEstoque: number, saldoEstoque: number) => {
  ...
  const calculateExpression = useCallback(
    (expression: string): { result: number; isValid: boolean; error?: string } => {
      try {
        ...
        if (!validPattern.test(cleanExpression)) {
          return { result: 0, isValid: false, error: "Caracteres inválidos na expressão" }
        }

        // Verifica se há operadores consecutivos (ex: ++, --, etc)
        const consecutiveOperators = /[+\-*/]{2,}/
        if (consecutiveOperators.test(cleanExpression)) {
          return { result: 0, isValid: false, error: "Operadores consecutivos não permitidos" }
        }

        // Verifica se começa ou termina com operador (exceto - no início)
        const startsWithOperator = /^[+*/]/
        const endsWithOperator = /[+\-*/]$/
        if (startsWithOperator.test(cleanExpression) || endsWithOperator.test(cleanExpression)) {
          return { result: 0, isValid: false, error: "Expressão não pode começar ou terminar com operador" }
        }
        ...
        return { result: 0, isValid: false, error: "Erro ao calcular expressão" }
      }
    },
    [],
  )
  ...
  const handleAddCount = useCallback(() => {
    ...
    if (existingIndex >= 0) {
      ...
      const expressionText = hasOperators ? ` (${quantityInput} = ${quantidade})` : ""
      toast({
        title: `Quantidade somada!`,
        description: `${quantidade} adicionado ao ${countingMode}${expressionText}. Total ${countingMode}: ${countingMode === "loja" ? existing.quant_loja : existing.quant_estoque}`,
      })
    } else {
      ...
      const expressionText = hasOperators ? ` (${quantityInput} = ${quantidade})` : ""
      toast({ title: `Quantidade de ${countingMode} adicionada!${expressionText}` })
    }
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      <header className="bg-white dark:bg-gray-800 shadow-sm border-b dark:border-gray-700">
        <Navigation showClearButton={true} onClearData={() => setShowClearDataModal(true)} />
      </header>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="quantity">
                      ...
                      <span className="text-xs text-gray-500 dark:text-gray-400 ml-2">(Ex: 24+24 ou 10*3)</span>
                    </Label>
                    <Input
                      id="quantity"
                      type="text"
                  ...
                  <div className="space-y-2 max-h-96 overflow-y-auto">
                    {productCounts.length === 0 ? (
                      ...
                      productCounts.map((item) => (
                        <ProductCountItem key={item.id} item={item} onRemove={handleRemoveCount} />
                      ))
                    )}
                  </div>
                </CardContent>
            ...
            <Card>
              <CardHeader>
                ...
                <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
                  ...
                  <ul className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
                    ...
                    <li>
                      • <strong>Código de barras:</strong> Formate a coluna como NÚMERO (não texto)
                    </li>
                    <li>
                    ...
                    <li>
                      • <strong>Cabeçalho:</strong> Primeira linha deve conter os nomes das colunas
                    </li>
                  </ul>
                  <div className="mt-3 flex items-center justify-between">
                    <div className="text-xs text-blue-600 dark:text-blue-400">
                      <strong>Exemplo:</strong> codigo_de_barras;codigo_produto;descricao;saldo_estoque
                      <br />
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="csv-file" type="file" accept=".csv" onChange={handleCsvUpload} disabled={isLoading} />
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {products.map((product) => {
                          ...
                          return <ProductTableRow key={product.id} product={product} barCode={barCode} />
                        })}
                      </TableBody>
                    </Table>
                  </div>
          ...
          <TabsContent value="export" className="space-y-6">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm mt-1">
                      A exportação de contagens está disponível apenas para assinantes premium. Faça upgrade para
                      exportar seus dados em CSV e PDF.
                    </p>
                  </div>
                  <Button
                    variant="outline"
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Visão geral do progresso da contagem atual</CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg text-center">
                    ...
                    <p className="text-sm text-blue-800 dark:text-blue-200">Produtos Importados</p>
                    <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                      {products.length} permanentes + {tempProducts.length} temporários
                    </p>
                  </div>

                  <div className="p-4 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-lg text-center">
                    <p className="text-2xl font-bold text-green-600 dark:text-green-400">{productCounts.length}</p>
                    <p className="text-sm text-green-800 dark:text-green-200">Produtos Contados</p>
                    <p className="text-xs text-green-600 dark:text-green-400 mt-1">
                      {productCountsStats.totalLoja + productCountsStats.totalEstoque} unidades totais
                    </p>
                  </div>
                  ...
                  <div className="p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg text-center">
                    <p className="text-2xl font-bold text-amber-600 dark:text-amber-400">
                      {Math.max(0, products.length + tempProducts.length - productCounts.length)}
                    </p>
                    <p className="text-sm text-amber-800 dark:text-amber-200">Itens Faltantes</p>
                    <p className="text-xs text-amber-600 dark:text-amber-400 mt-1">
                      {productCounts.length === 0
                        ? "Nenhuma contagem iniciada"
                        : `${Math.round((productCounts.length / (products.length + tempProducts.length)) * 100)}% concluído`}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="mt-4">
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <span className="text-sm text-gray-500 dark:text-gray-400">
                        {productCounts.length} de {products.length + tempProducts.length}
                      </span>
                    </div>
                    <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2">
                      <div
                        ...
                        style={{
                          width: `${Math.min(100, (productCounts.length / (products.length + tempProducts.length)) * 100)}%`,
                        }}
                      ></div>
                    </div>
                  </div>
                ...
                {products.length + tempProducts.length > 0 && productCounts.length === 0 && (
                  <Alert className="mt-4 border-blue-200 bg-blue-50 dark:border-blue-800 dark:bg-blue-900/20">
                    ...
                    <AlertDescription className="text-blue-800 dark:text-blue-200">
                      ...
                      <p className="text-sm mt-1">
                        Você tem {products.length + tempProducts.length} produtos importados. Vá para a aba
                        "Conferência" para começar a contagem.
                      </p>
                    </AlertDescription>
                  </Alert>
                )}
                ...
                {products.length + tempProducts.length > 0 &&
                  productCounts.length > 0 &&
                  productCounts.length < products.length + tempProducts.length && (
                    <Alert className="mt-4 border-amber-200 bg-amber-50 dark:border-amber-800 dark:bg-amber-900/20">
                      ...
                      <AlertDescription className="text-amber-800 dark:text-amber-200">
                        ...
                        <p className="text-sm mt-1">
                          Ainda restam {products.length + tempProducts.length - productCounts.length} produtos para
                          contar. Continue na aba "Conferência" para completar a contagem.
                        </p>
                      </AlertDescription>
                    </Alert>
                  )}
                  ...
                  productCounts.length >= products.length + tempProducts.length && (
                    <Alert className="mt-4 border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-900/20">
                      ...
                      <AlertDescription className="text-green-800 dark:text-green-200">
                        ...
                        <p className="text-sm mt-1">
                          Todos os produtos foram contados. Você pode exportar os dados ou continuar adicionando mais
                          contagens.
                        </p>
                      </AlertDescription>
                    </Alert>
            ...
            <Card className="opacity-60">
              ...
              <CardContent className="space-y-4">
                <div className="grid gap-4 md:grid-cols-2">
                  <Button onClick={exportToCsv} disabled={true} className="h-12">
